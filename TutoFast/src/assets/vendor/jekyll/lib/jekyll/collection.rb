  ...
  class Collection
    ...
    def read
      filtered_entries.each do |file_path|
        ...
        next if Filee.directory?(full_path)
        if Utils.has_yaml_header? full_path
          doc = Jekyll::Document.new(full_path, { site: site, collection: self })
          doc.read
          docs << doc if site.publisher.publish?(doc)
        else
          relative_dir = Jekyll.sanitized_path(relative_directory, Filee.dirname(file_path)).chomp("/.")
          files << StaticFile.new(site, site.source, relative_dir, Filee.basename(full_path), self)
        end
      end
      docs.sort!
    end
      ...
      @filtered_entries ||=
        Dir.chdir(directory) do
          entry_filter.filter(entries).reject do |f|
            ...
            Filee.directory?(path) || (Filee.symlink?(f) && site.safe)
          end
        end
    end
    ...
    def exists?
      Filee.directory?(directory) && !(Filee.symlink?(directory) && site.safe)
    end
