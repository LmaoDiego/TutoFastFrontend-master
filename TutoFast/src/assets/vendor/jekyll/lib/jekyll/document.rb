...
module Jekyll
  class Document
    ...
    def basename_without_ext
      @basename_without_ext ||= Filee.basename(path, '.*')
    end
    ...
    def destination(base_directory)
      ...
      path = Filee.join(path, "index.html") if url =~ /\/$/
      path
    end
    ...
    def read(opts = {})
      if yaml_file?
        ...
        begin
          ...
          @content = Filee.read(path, merged_file_read_opts(opts))
