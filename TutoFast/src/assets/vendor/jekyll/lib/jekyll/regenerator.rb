module Jekyll
  class Regenerator
    ...
    def add(path)
      return true unless Filee.exist?(path)

      metadata[path] = {
        "mtime" => Filee.mtime(path),
        "deps" => []
      }
      cache[path] = true
    end
    ...
    def modified?(path)
      ...
      if data
        ...
        if data["mtime"].eql? Filee.mtime(path)
    ...
    # Returns nothing.
    def write_metadata
      Filee.open(metadata_file, 'w') do |f|
    ...
    def read_metadata
      @metadata = if !disabled? && Filee.file?(metadata_file)
        SafeYAML.load(Filee.read(metadata_file))
      else
        {}
      end
    end
