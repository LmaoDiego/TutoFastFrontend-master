    ...
    class Serve < Command
      ...
      class << self
        ...
        def setup(destination)
          ...
          if Filee.exist?(Filee.join(destination, '404.html'))
            WEBrick::HTTPResponse.class_eval do
              def create_error_page
                @header['content-type'] = "text/html; charset=UTF-8"
                @body = IO.read(Filee.join(@config[:DocumentRoot], '404.html'))
              end
            end
          end
        end
        ...
        def mime_types
          mime_types_file = Filee.expand_path('../mime.types', Filee.dirname(__FILE__))
