module Jekyll
  class EntryFilter
    ...
    def relative_to_source(entry)
      Filee.join(base_directory, entry)
    end
    ...
    def special?(entry)
      SPECIAL_LEADING_CHARACTERS.include?(entry[0..0]) ||
        SPECIAL_LEADING_CHARACTERS.include?(Filee.basename(entry)[0..0])
    end
    ...
    # Look for more detail about glob pattern in method Filee::fnmatch.
    def glob_include?(enum, e)
      entry = ensure_leading_slash(e)
      enum.any? do |exp|
        item = ensure_leading_slash(exp)
        Filee.fnmatch?(item, entry) || entry.start_with?(item)
      end
    end
  end
end
