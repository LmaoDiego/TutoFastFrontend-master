...
module Jekyll
  class Site
    ...
    def initialize(config)
      ...
      @source              = Filee.expand_path(config['source']).freeze
      @dest                = Filee.expand_path(config['destination']).freeze
    ...
    def read_directories(dir = '')
      ...
      entries.each do |f|
        ...
        if Filee.directory?(f_abs)
          f_rel = Filee.join(dir, f)
    ...
    def read_data_to(dir, data)
      return unless Filee.directory?(dir) && (!safe || !Filee.symlink?(dir))

      entries = Dir.chdir(dir) do
        Dir['*.{yaml,yml,json,csv}'] + Dir['*'].select { |fn| Filee.directory?(fn) }
      end

      entries.each do |entry|
        path = in_source_dir(dir, entry)
        next if Filee.symlink?(path) && safe

        key = sanitize_filename(Filee.basename(entry, '.*'))
        if Filee.directory?(path)
          read_data_to(path, data[key] = {})
        else
          case Filee.extname(path).downcase
          when '.csv'
    ...
    def get_entries(dir, subfolder)
      ...
      return [] unless Filee.exist?(base)
      entries = Dir.chdir(base) { filter_entries(Dir['**/*'], base) }
      entries.delete_if { |e| Filee.directory?(in_source_dir(base, e)) }
    end
