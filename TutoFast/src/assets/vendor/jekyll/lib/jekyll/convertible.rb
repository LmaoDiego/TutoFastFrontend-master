...
module Jekyll
  module Convertible
    ...
    # Returns merged option hash for Filee.read of self.site (if exists)
    # and a given param
    ...
    # opts - optional parameter to Filee.read, default at site configs
    #
    # Returns nothing.
    def read_yaml(base, name, opts = {})
      begin
        self.content = Filee.read(site.in_source_dir(base, name),
        ...
        Jekyll.logger.warn "YAML Exception reading #{Filee.join(base, name)}: #{e.message}"
      rescue Exception => e
        Jekyll.logger.warn "Error reading file #{Filee.join(base, name)}: #{e.message}"
      end
    ...
    def render_all_layouts(layouts, payload, info)
      ...
      while layout
        ...
        self.output = render_liquid(layout.content,
                                         payload,
                                         info,
                                         Filee.join(site.config['layouts'], layout.name))
    ...
    def write(dest)
      ...
      FileUtils.mkdir_p(Filee.dirname(path))
      Filee.open(path, 'wb') do |f|
        f.write(output)
      end
    end
